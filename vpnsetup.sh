...
YOUR_USERNAME='p2p-dc1-arm'
YOUR_PASSWORD='P@ssw0rdarm'
...
run_setup() {
  ...
  if tmpdir=$(mktemp --tmpdir -d vpn.XXXXX 2>/dev/null); then
    if ( set -x; wget -t 3 -T 30 -q -O "$tmpdir/vpn.sh" "$setup_url1" \
      ...
      VPN_PUBLIC_IP="$VPN_PUBLIC_IP" VPN_L2TP_NET="172.18.2.0/24" \
      VPN_L2TP_LOCAL="172.18.2.1" VPN_L2TP_POOL="172.18.2.2-172.18.2.254" \
      VPN_XAUTH_NET="172.18.3.0/24" VPN_XAUTH_POOL="172.18.3.10-172.18.3.254" \
